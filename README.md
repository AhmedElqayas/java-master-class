...
2. Create a package with your name and move `com.qayas.Main.java` inside it:

```
src/com/<yourname>/com.qayas.Main.java
```

For example, if your name is Franco: `src/com/franco/com.qayas.Main.java`

The starter `com.qayas.Main.java` contains TODOs to guide the first steps:
...
graph TB
    subgraph Presentation
        M[com.qayas.Main.java<br>CLI Menu Loop]
    end
...
- Implement the CLI menu loop in `com.qayas.Main.java`
...
**Wiring in com.qayas.Main.java** — the student can now swap between array and file implementations for bookings:
...
These can be swapped in via dependency injection in `com.qayas.Main.java`:
...
Place this bean in the `com.qayas.Main` class (annotated with `@SpringBootApplication`). The data is inserted into H2 on every startup since the database is in-memory.
...
│   │   └── com/
│   │       └── <yourname>/
│   │           ├── com.qayas.Main.java                       (@SpringBootApplication)
│   │           ├── car/
│   │           │   ├── Brand.java
│   │           │   ├── Car.java                    (@Entity)
